...
Y una tercera pieza que es la que hace la restriccion REAL: ``create_alert``
exige declarar que estrategia (E01-E12) se esta aplicando, y valida que exista.
Sin eso, "opera solo las del manual" seria una sugerencia que el modelo puede
ignorar en cualquier llamada.
"""
...
def bloque_prompt() -> str:
    ...
    return (
        ...
        "Al llamar create_alert es obligatorio el parametro ``estrategia`` con "
        "el codigo (p.ej. 'E01') y en ``thesis`` debes citar que condicion "
        "concreta se cumplio. Una alerta sin estrategia valida se rechaza.\n\n"
        ...
        "volatilidad', 'tendencia clara' ni 'terreno suficiente'. Ante un vacio "
        "NO inventes un numero: descarta la operacion y dilo."
    )
